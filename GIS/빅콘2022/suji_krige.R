...
map1 = leaflet(charger_수지) %>% 
  ...
  addCircles(lng = ~lng, lat = ~lat, color = ~pal(charger_수지$count))
map1  
...
testdat3$result <- (testdat3$predict_y - testdat3$predict_bc)
testdat3 %>% filter(result < 0) %>% nrow 
testdat3 %>% filter(result < min(result)+0.25)
testdat4 <- testdat3[,c('lon', 'lat', 'result')]
write.csv(testdat4, 'scenario.csv')
