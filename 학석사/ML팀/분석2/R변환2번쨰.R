...
variables <- columns[4:21]
target <- columns[2]

data_FS_x <- data[variables] # 독립변수만 있는 데이터
data_FS_y <- data[target] # 종속변수만 있는 데이터
...
x_train <- data_FS_x[idx,][,1:17]
x_test <- data_FS_x[-idx,][,1:17]
y_train <- data_FS_y[idx,]
...
control <- rfeControl(functions = rfFuncs, method = 'repeatedcv', 
                      repeats = 3, number = 10)
rfe1 <- rfe(x = x_train, y = y_train, size = c(1:17), rfeControl = control);rfe1
RMSE <- rfe1$results[[2]] # 각 변수의 RMSE값
...
# RMSE순으로 rank정렬
library(dplyr)
cell27 <- cbind(variables, RMSE2, rank(RMSE2))
